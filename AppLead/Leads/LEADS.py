...
df['countSub'] = df.groupby(['SERVICE', 'TRAFFIC_SOURCE', 'cutted_zip_code'])['IS_SUBMITTED'].transform(lambda x: x.sum())

#вычисляем конверсию на агрегате
df['successRateSub'] = df['countSub']/df['countAggreg']
...
df = assign_traffic_source_and_recommendation(df, countAggregSub_rec, successRateSub_rec)

print('not recomended Agregs searching...')

df = assign_not_recommended(df, countAggregSub_not_rec, successRateSub_not_rec)
...
if duplicate_agreg_drop:
    df = df.drop_duplicates(subset='Agreg_ID', keep='first')

filename = input('Input file name\n')
print('Writing to file...')
df.to_excel(filename)
input('File was successfully generated, press ENTER to exit\n')
#print(df_test, '\n')
#print(df_train)
